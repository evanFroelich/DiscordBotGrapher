    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.tripleDown:
            if result == 1:
                ...
                games_curs.execute('''INSERT INTO DailyGamblingTotals (Date, Category, Funds) Values (?, ?, ?) ON CONFLICT(Date, Category) DO UPDATE SET Funds = Funds + ?;''', (datetime.now().strftime('%Y-%m-%d'), 'Alleyway', self.wager*2, self.wager*2))
                games_conn.commit()
                games_curs.close()
                games_conn.close()
            else:
            ...
            games_curs.execute('''INSERT INTO DailyGamblingTotals (Date, Category, Funds) Values (?, ?, ?) ON CONFLICT(Date, Category) DO UPDATE SET Funds = Funds + ?;''', (datetime.now().strftime('%Y-%m-%d'), 'Alleyway', self.wager, self.wager))
            games_conn.commit()
            games_curs.close()
            games_conn.close()
            self.streak += 1
